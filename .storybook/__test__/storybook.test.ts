...
function getAllStoryFiles() {
  ...
  const p = path.join(__dirname, '../../src/**/**/*.@(stories|story).@(js|jsx|mjs|ts|tsx)')
...
describe(options.suite, () => {
  const allStoryFiles = getAllStoryFiles()
  allStoryFiles.forEach(({ storyFile, filePath }) => {
    ...
    describe(title, () => {
      ...
      stories.forEach(({ name, story }) => {
        ...
        testFn(name, async () => {
          ...
          // FIXME: low priority reason: not production code
          // @ts-ignore
